...
#include "visualime/cuda/cuda_opengl_wrapper.cuh"
...
namespace visualime {
    ...
    public:
        ...
        cuda_scene2d(unsigned int width, unsigned int height, double super_sampling_ratio = 1.0, bool fullscreen = false):
                _width(width), _height(height),
                _engine(static_cast<unsigned>(width * super_sampling_ratio) ,
                        static_cast<unsigned>(height * super_sampling_ratio)),
                _background_canvas(std::make_shared<canvas::fullscreen_canvas>
        ...
        void run(bool show_fps = false) {
            ...
            while (!_opengl_wrapper.should_close()) {
                ...
                _engine.copy_to(_opengl_wrapper.get_ptr());
                _opengl_wrapper.render_call(nullptr);
                _opengl_wrapper.swap_buffers();
                _opengl_wrapper.poll_events();
                counter += 1;
            }
        ...
        void launch(bool show_fps = false) {
            _running = true;
            _run_thread = std::thread(&cuda_scene2d::run, this, show_fps);
        }
    ...
    private:
        ...
        unsigned int _height;
        std::vector<std::shared_ptr<primitive::primitive_base>> _primitives;
        engine::cuda_engine _engine;
        opengl_wrapper::cuda_opengl_wrapper _opengl_wrapper;
        bool _scene_changed = false;
        bool _running = false;
        std::thread _run_thread;
        std::mutex _mutex;
